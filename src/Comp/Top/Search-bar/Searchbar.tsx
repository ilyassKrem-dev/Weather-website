...
export default function SearchBar(props:any) {
    ...
    // Function to check if a city name is already in the Set
    const isCityNameAlreadyInSet = (cityName:string) => {
        if (uniqueCityNamesSet.has(cityName)) {
            return true; // City name is already in the Set
        } else {
            uniqueCityNamesSet.add(cityName); // Add the city name to the Set
            return false; // City name is not in the Set
        }
    };
        ...
        <>
            <div className="flex justify-center mb-5">
                <div className="relative">
                    <div>
                        <div className="absolute left-2 top-[0.2rem] cursor-pointer">
                        ...
                        <input onClick={() => setShowResults(true)} onChange={handleChange} value={userInput} className="bg-white rounded-[25px] h-[2.2rem] w-[25rem] px-2 pl-11 outline-none font-[600]" type="text" placeholder="Search location..."/>
                    </div>
